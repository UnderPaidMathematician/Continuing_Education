...
class Deck():
  ...
  def GetCards(self):
    ret = []
    for i in range(self.GetSize()-1, -1 ,-1): #This will count backwards, yes?
        ret.append(self.deck[i]) #Adding backwards since we consider the last card in this.deck to be on top
    return ret
